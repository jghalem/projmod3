...
public class ReservaDAO extends DAO {
    ...
    public void inserir(Reserva r) throws Exception {
        abrirConexao();
        stmt = con.prepareStatement("insert into reserva r,quarto q (r.numeroreserva,r.cpf,r.numeroquarto,r.checkin,r.checkout,q.dispQuarto) values(?,?,?,?,?,?)");
        stmt.setInt(1, r.getNumeroReserva());
        stmt.setInt(2, r.getCpf());
        stmt.setInt(3, r.getNumeroQuarto());
        stmt.setTimestamp(4, r.getCheckIn());
        stmt.setTimestamp(5, r.getCheckOut());

        dispQ(r.getNumeroQuarto(), false);
    ...
    public void dispQ(Integer numeroQuarto, boolean disponibilidadeQuarto) throws Exception {
        abrirConexao();
	...
	public String atualizarReserva(Reserva r) throws Exception {
		try {
			abrirConexao();
			stmt = con.prepareStatement("update reserva set numeroreserva = ?, cpf = ?, numeroquarto = ? where numeroreserva = ?;");
	        stmt.setInt(1, r.getNumeroReserva());
	        stmt.setInt(2, r.getCpf());
	        stmt.setInt(3, r.getNumeroQuarto());
